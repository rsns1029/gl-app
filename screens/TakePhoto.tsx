...
const TakePhoto = ({navigation}: PhotoNavigationProps) => {
  ...
  return (
    <Container>
      ...
      {isFocused === true && takenPhotoUri !== '' ? (
        <TakenPhotoPreview source={{uri: takenPhotoUri}} />
      ) : null}
      {takenPhotoUri === '' ? (
        <ActionContainer>
          ...
          <FlashContainer onPress={handleWhiteBalance}>
            ...
            {whiteBalance === Camera.Constants.WhiteBalance.fluorescent && (
              <Ionicons name={'md-cloudy-night'} size={24} color="white" />
            )}
          </FlashContainer>
        </ActionContainer>
      ) : null}
      {takenPhotoUri === '' ? (
        <CameraButtons>
          <TouchableOpacity onPress={handleNavigateToSelectPhotoScreen}>
            {previewPhoto && <PreviewPhotoImage source={{uri: previewPhoto}} />}
          </TouchableOpacity>
